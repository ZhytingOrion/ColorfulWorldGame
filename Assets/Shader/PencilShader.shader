Shader "Unlit/Pencil"
{
	Properties
	{
		...
		//描边系数
		_OutlineFactor("OutlineFactor",Range(0.0,0.01)) = 0.01
	}
		SubShader
		{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float lum = (hatchColor.r + hatchColor.g + hatchColor.b) / 3;
				float4 color = tex2D(_MainTex, i.uv_main);
				float3 finalColor = (1 - lum) * color.rgb + hatchColor.rgb;
				...
				return float4(finalColor * atten, 1.0);
			}
			ENDCG
		}
		}
